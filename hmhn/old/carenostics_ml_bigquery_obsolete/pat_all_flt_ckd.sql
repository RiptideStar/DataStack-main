...
\time_as 'insert into pat_all_flt_ckd'
...
set 
  ...
  ckd_label_date = least(
    coalesce(
      ckd_first_dx_3_plus_date, datetime '2100-01-01'
    ), 
    coalesce(
      ckd_egfr_diagnosis_date, datetime '2100-01-01'
    ), 
    coalesce(
      ckd_first_abn_uacr_date, datetime '2100-01-01'
    )
  ) 
where 
  ckd_egfr 
...
\time_as 'compute kidney transplant'
update 
  pat_all_flt_ckd 
set 
...
where 
 `{dataset}`.eligibleForCohort(birth_date, living, dialysis_dx, kidney_transplant_dx)
  and not ckd_label 
  and (
    number_of_uacrs_post_last_normal > 0 
    or number_of_egfrs_post_last_normal > 0
  ) 
  ;
 ...
 `{dataset}`.eligibleForCohort(birth_date, living, dialysis_dx, kidney_transplant_dx)
  ...
  and not exists (
    ...
    where 
      ...
      and discon_time is NULL
  ) ;

\intfmt ,
select count(*) `pat_all_flt_ckd size` from pat_all_flt_ckd;
